...
// pdfminer sometimes reverses entire Hebrew strings (RTL artifact).
// "גולדאל הנדסה" → "הסדנה לאדלוג" in extracted text.
...
function extractCompanyName(firstPageText, profiles) {
  for (const name of Object.keys(profiles)) {
    ...
    // Normal match
    if (firstPageText.includes(prefix)) return name;
    // RTL-flipped match: check first 8 chars of the fully-reversed name
    const flipped = _reverseStr(name);
    if (firstPageText.includes(flipped.slice(0, Math.min(8, flipped.length)))) return name;
  }
